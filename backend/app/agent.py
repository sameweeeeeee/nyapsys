...
SYSTEM_MESSAGE = (
    ...
    "and understand images. Be concise but thorough. If you are unsure, say so."
)
...
async def run(
    ...
    image_ingest_result = None

    if file_bytes and filename and file_type:
        if ingest.is_image_type(file_type):
            image_ingest_result = await ingest.ingest_image(
                file_bytes, filename, file_type, conversation_id
            )
        else:
            await ingest.ingest_file(
                file_bytes, filename, file_type, conversation_id
            )

    context = ""
    if not image_ingest_result:
    ...
    if image_ingest_result:
        async for token in model.generate_with_image(
            text=user_message,
    ...
    await db.insert_message(
        ...
        has_image=bool(image_ingest_result),
    )
