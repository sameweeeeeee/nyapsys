...
async def ingest_file(
    ...
    if ingest_module.is_image_type(file_type):
        result = await ingest_module.ingest_image(
        ...
        return IngestResponse(
            file_id=result.file_id, chunk_count=result.chunk_count, filename=result.filename
        )
...
async def submit_feedback(feedback: FeedbackCreate):
    ...
    await db.insert_eval(eval_id, feedback.message_id, feedback.score, feedback.feedback)
    return {"id": eval_id}